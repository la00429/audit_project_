...
async function handleAuditRequest(sendResponse) {
    try {
        ...
        sendResponse({ type: 'AUDIT_ERROR', payload: error.message });
    }
}
// --- Local Audit Engine (all 7 WCAG rules) ---
function runLocalAudit(elements, pageUrl) {
    ...
    for (const el of elements) {
        if (!el.textContent?.trim() || !el.computedStyles?.['color'] || !el.computedStyles?.['backgroundColor'])
            continue;
