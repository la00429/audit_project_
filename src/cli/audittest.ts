...
function getScoreColor(score: number): string {
  ...
  if (score >= 50) return '\x1b[38;5;208m'; // orange
  return '\x1b[31m'; // red
}
...
function printScoreBar(score: number) {
...
function resolveUrl(input: string): string {
  if (input.startsWith('http') || input.startsWith('file://')) return input;
...
async function runSingleAudit(url: string, takeScreenshot: boolean): Promise<AuditResult> {
  ...
  if (takeScreenshot) {
    const buffer = await page.screenshot({ fullPage: false, encoding: 'binary' }) as Buffer;
    screenshotBase64 = buffer.toString('base64');
    if (flags.screenshot) fs.writeFileSync('audittest-screenshot.png', buffer);
  }
...
async function main() {
  ...
  // --- DIFF MODE ---
  if (flags.diff) {
    if (!diffUrl) { printError('--diff requiere una segunda URL. Uso: npx audittest-vision url1 --diff url2'); process.exit(1); }
...
async function runWatchMode(url: string) {
  let previousIssues: AuditIssue[] = [];
  let cycle = 0;
  ...
  const runCycle = async () => {
    ...
    try {
      ...
      console.log(`  \x1b[31m✖ Error: ${(e as Error).message}\x1b[0m\n`);
    }
  };
...
async function runDiffMode(url1: string, url2: string) {
  ...
  console.log(`  Score: `); printScoreBar(result1.score);
  console.log(`\n  \x1b[1mURL 2:\x1b[0m ${url2}`);
  console.log(`  Score: `); printScoreBar(result2.score);
    ...
    for (const tag of tags) {
      document.querySelectorAll(tag).forEach((el: Element) => {
        ...
        elements.push({
          ...
          attributes: { alt: el.getAttribute('alt') || '', 'aria-label': el.getAttribute('aria-label') || '', 'aria-labelledby': el.getAttribute('aria-labelledby') || '', role: el.getAttribute('role') || '', id: (el as HTMLElement).id || '', type: el.getAttribute('type') || '', href: el.getAttribute('href') || '' },
...
function printStep(msg: string) { if (!flags.json) console.log(`  \x1b[36m►\x1b[0m ${msg}`); }
function printError(msg: string) { console.error(`  \x1b[31m✖\x1b[0m ${msg}`); }
...
function printResults(result: AuditResult) {
  ...
  if (issues.length === 0) {
    ...
    for (const issue of issues) {
      const colors: Record<string, string> = { critical: '\x1b[31m', major: '\x1b[33m', minor: '\x1b[34m', info: '\x1b[90m' };
      console.log(`  ${colors[issue.severity]}[${issue.severity.toUpperCase()}]\x1b[0m ${issue.title}`);
      console.log(`    ${issue.description}`);
      if (issue.fix) console.log(`    \x1b[32m💡 ${issue.fix}\x1b[0m`);
      console.log('');
    }
  }

  console.log('\x1b[35m──────────────────────────────────────────────────────────\x1b[0m');
  if (critical > 0) console.log(`  \x1b[31m✖ FALLO: ${critical} issue(s) crítico(s)\x1b[0m`);
  else console.log(`  \x1b[32m✓ PASÓ: Sin issues críticos\x1b[0m`);
  console.log('');
}
