...
interface WcagRule {
  ...
  criterion: string;
  level: 'A' | 'AA' | 'AAA';
  evaluate: (elements: DOMElementMeta[]) => WcagIssue[];
}
...
export class WcagModule {
  ...
  async evaluate(elements: DOMElementMeta[]): Promise<WcagIssue[]> {
  ...
  private missingAltTextRule(): WcagRule {
    return {
      id: 'missing-alt',
      criterion: '1.1.1',
      level: 'A',
  ...
  private colorContrastRule(): WcagRule {
    return {
      id: 'color-contrast',
      criterion: '1.4.3',
      level: 'AA',
  ...
  private missingFormLabelRule(): WcagRule {
    return {
      id: 'missing-label',
      criterion: '1.3.1',
      level: 'A',
  ...
  private headingHierarchyRule(): WcagRule {
    return {
      id: 'heading-order',
      criterion: '1.3.1',
      level: 'A',
  ...
  private missingLandmarkRule(): WcagRule {
    return {
      id: 'missing-landmark',
      criterion: '1.3.1',
      level: 'A',
      evaluate: (elements) => {
        ...
        const hasLandmark = elements.some(
          el => landmarks.includes(el.tagName.toLowerCase()) || !!el.attributes?.['role']
        );
  ...
  private isLargeText(el: DOMElementMeta): boolean {
